ladies, mens, baby_kids, interior_housing_accessory, book_music_game, toy_hobby_goods, cosmetics_beauty_perfumem, appliances_smartphone_camera, sports_leisure, handmade, ticket, car_bike, other = Category.create([
  {:name => 'レディース'},
  {:name => 'メンズ'},
  {:name => 'ベビー・キッズ'},
  {:name => 'インテリア・住まい・小物'},
  ...
  # レディース
ladies_children_array = ['トップス', 'ジャケット/アウター', 'パンツ', 'スカート', 'ワンピース', '靴', 'ルームウェア/パジャマ', 'レッグウェア', '帽子', 'バッグ', 'アクセサリー', 'ヘアアクセサリー', '小物', '時計', 'ウェッグ/エクステ', '浴衣/水着', 'スーツ/フォーマル/ドレス', 'マタニティ', 'その他']
ladies_grandchildren_array = [
  ...
  ['前髪ウェッグ', 'ロングストレート', 'ロングカール', 'ショートストレート', 'ショートカール', 'その他'],
  ['浴衣', '着物', '振袖', '長襦袢/半襦袢', '水着セパレート', '水着ワンピース', '水着スポーツ用'],
  ['スカートスーツ上下', 'パンツスーツ上下', 'ドレス', 'パーティーグッズ', 'シューズ', 'ウェディング', 'その他'],
...
ladies_children_array.each_with_index do |children, index|
  ...
  # メンズ
mens_children_array = ['トップス', 'ジャケット/アウター', 'パンツ', '靴', 'バッグ', 'スーツ', '帽子', 'アクセサリー', '小物', '時計', '水着', 'レッグウェア', 'アンダーウェア']
...
mens_children_array.each_with_index do |children, index|
  ...
  # ベビー・キッズ
baby_kids_children_array = ['ベビー服(女の子用)~95cm', 'ベビー服(男の子用)~95cm', 'ベビー服(男女兼用)~95cm', 'キッズ服(女の子用)100cm~', 'キッズ服(男の子用)100cm~', 'キッズ服(男女兼用)100cm~', 'キッズ靴', '子ども用ファッション小物', 'おむつ/トイレ/バズ', '外出/移動用品', '授乳/食事', 'ベビー家具/寝具/室内用品', 'おもちゃ', '行事/記念品', 'その他']
baby_kids_grandchildren_array = [
  ['トップス', 'アウター', 'パンツ', 'スカート', 'ワンピース', 'ベビードレス', 'おくるみ', '下着/肌着', 'パジャマ', 'ロンパース', 'その他'],
  ['トップス', 'アウター', 'パンツ', 'おくるみ', '下着/肌着', 'パジャマ', 'ロンパース', 'その他'],
  ['トップス', 'アウター', 'パンツ', 'おくるみ', '下着/肌着', 'パジャマ', 'ロンパース', 'その他'],
  ['コート', 'ジャケット/上着', 'トップス(Tシャツ/カットソー)', 'トップス(トレーナー)', 'トレーナー(チュニック)', 'トップス(タンクトップ)', 'トップス(その他)', 'スカート', 'パンツ', 'ワンピース', 'セットアップ', 'パジャマ', 'フォーマル/ドレス', '和服', '浴衣', '甚平', '水着', 'その他'],
...
baby_kids_children_array.each_with_index do |children, index|
  children = baby_kids.children.create(name: children)
  baby_kids_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # インテリア・住まい・小物
interior_housing_accessory_children_array = ['キッチン/食器', 'ベッド/マットレス', 'ソファ/ソファベッド', '椅子/チェア', '机/テーブル', '収納家具', 'ラグ/カーペット/マット', 'カーテン/ブラインド', 'ライト/照明', '寝具', 'インテリア小物', '季節/年中行事']
interior_housing_accessory_grandchildren_array = [
  ['食器', '調理器具', '収納/キッチン雑貨', '弁当用品', 'カトラリー(スプーン等)', 'テーブル用品', '容器', 'エプロン', 'アルコールグッズ', '浄水器', 'その他'],
  ...
  ['ソファセット', 'シングルソファ', 'ラブソファ', 'トリプルソファ', 'オットマン', 'コーナーソファ', 'ビーズソファ/クッションソファ', 'ローソファ/フロアソファ', 'ソファベッド', '応援セット', 'ソファカバー', 'リクライニングソファ', 'その他'],
  ...
  ['正月', '成人式', 'バレンタインデー', 'ひな祭り', 'こどもの日', '母の日', '父の日', 'サマーギフト/お中元', '夏/夏休み', 'ハロウィン', '敬老の日', '七五三', 'お歳暮', 'クリスマス', '冬一般', 'その他']
]

interior_housing_accessory_children_array.each_with_index do |children, index|
  children = interior_housing_accessory.children.create(name: children)
  interior_housing_accessory_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # 本・音楽・ゲーム
book_music_game_children_array = ['本', '漫画', '雑誌', 'CD', 'DVD/ブルーレイ', 'レコード', 'テレビゲーム']
book_music_game_grandchildren_array = [
  ...
  ['邦楽', '洋楽', 'その他'],
  ['家庭ゲーム本体', '家庭用ゲームソフト', '携帯用ゲーム本体', '携帯用ゲームソフト', 'PCゲーム', 'その他']
]

book_music_game_children_array.each_with_index do |children, index|
  children = book_music_game.children.create(name: children)
  book_music_game_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  #おもちゃ・ホビー・グッズ
toy_hobby_goods_children_array = ['おもちゃ', 'タレントグッズ', 'コミック/アニメグッズ', 'トレーディングカード', 'フィギュア', '楽器/器材', 'コレクション', 'ミリタリー', '美術品', 'アート用品', 'その他']
toy_hobby_goods_grandchildren_array = [
  ...
  ['遊戯王', 'マジック：ザ：ギャザリング', 'ポケモンカードゲーム', 'デェエルマスターズ', 'バトルスピリッツ', 'プリパラ', 'アイカツ', 'カードファイト!!ヴァンガード', 'ヴァイスシュヴァルツ', 'プロ野球オーナーズリーグ', 'ベースボールヒーローズ', 'ドラゴンボール', 'スリーブ', 'その他'],
...
toy_hobby_goods_children_array.each_with_index do |children, index|
  children = toy_hobby_goods.children.create(name: children)
  toy_hobby_goods_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # コスメ・香水・美容
cosmetics_beauty_perfumem_children_array = ['ベースメイク', 'メイクアップ', 'ネイルケア', '香水', 'スキンケア/基礎化粧品', 'ヘアケア', 'ボディケア', 'オーラルケア', 'リラクゼーション', 'ダイエット', 'その他']
cosmetics_beauty_perfumem_grandchildren_array = [
  ['ファンデーション', '化粧下地', 'コントロールカラー', 'BBクリーム', 'CCクリーム', 'コンシーラー', 'フェイスパウダー', 'トライアルセット/サンプル', 'その他'],
  ['アイシャドウ', '口紅', 'リップグロス', 'リップライナー', 'チーク', 'フェイスカラー', 'マスカラ', 'アイライナー', 'つけまつげ', 'アイブロウペンシル', 'パウダーアイブロウ', '眉マスカラ', 'トライアルセット/サンプル', 'メイク道具/化粧小物', '美顏用品/美顏ローラー', 'その他'],
...
cosmetics_beauty_perfumem_children_array.each_with_index do |children, index|
  children = cosmetics_beauty_perfumem.children.create(name: children)
  cosmetics_beauty_perfumem_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # 家電・スマホ・カメラ
appliances_smartphone_camera_children_array = ['スマートフォン/携帯電話', 'スマホアクセサリー', 'PC/タブレット', 'カメラ', 'テレビ/映像機器', 'オーディオ機器', '美容/健康', '冷暖房/空調', '生活家電']
appliances_smartphone_camera_grandchildren_array = [
  ...
  ['テレビ', 'プロジェクター', 'ブルーレイレコーダー', 'DVDレコーダー', 'ブルーレイプレーヤー', 'DVDプレーヤー', '映像ケーブル', 'その他'],
...
appliances_smartphone_camera_children_array.each_with_index do |children, index|
  children = appliances_smartphone_camera.children.create(name: children)
  appliances_smartphone_camera_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # スポーツ・レジャー
sports_leisure_children_array = ['ゴルフ', 'フィッシング', '自転車', 'トレーニング/エクササイズ', '野球', 'サッカー/フットサル', 'テニス', 'スノーボード', 'スキー', 'その他スポーツ', 'アウトドア', 'その他']
sports_leisure_grandchildren_array = [
  ['クラブ', 'ウェア(男性用)', 'ウェア(女性用)', 'バッグ', 'シューズ(男性用)', 'シューズ(女性用)', 'アクセサリー', 'その他'],
  ...
  ['ラケット(硬式用)', 'ラケット(硬式用)', 'ウェア', 'シューズ', 'ボール', 'アクセサリー', '記念グッズ', '応援グッズ', 'その他'],
  ['ボード', 'バインディング', 'ブーツ(男性用)', 'ブーツ(女性用)', 'ブーツ(子ども用)', 'ウェア/装備(男性用)', 'ウェア/装備(女性用)', 'ウェア/装備(子ども用)', 'アクセサリー', 'バッグ', 'その他'],
  ['板', 'ブーツ(男性用)', 'ブーツ(女性用)', 'ブーツ(子ども用)', 'ビンディング', 'ウェア(男性用)', 'ウェア(女性用)', 'ウェア(子ども用)', 'ストック', 'その他'],
...
sports_leisure_children_array.each_with_index do |children, index|
  children = sports_leisure.children.create(name: children)
  sports_leisure_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # ハンドメイド
handmade_children_array = ['アクセサリー(女性用)', 'ファッション/小物', 'アクセサリー/時計', '日用品/インテリア', '趣味/おもちゃ', 'キッズ/ベビー', '素材/材料', '二次創作物']
handmade_grandchildren_array = [
...
handmade_children_array.each_with_index do |children, index|
  children = handmade.children.create(name: children)
  handmade_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # チケット
ticket_children_array = ['音楽', 'スポーツ', '演劇/芸能', 'イベント', '映画', '施設利用券', 'その他']
ticket_grandchildren_array = [
  ['男性アイドル', '女性アイドル', '韓流', '国内アーティスト', '海外アーティスト', '音楽フェス', '声優/アニメ'],
...
ticket_children_array.each_with_index do |children, index|
  children = ticket.children.create(name: children)
  ticket_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # 自動車・オートバイ
car_bike_children_array = ['自動車本体', '自動車タイヤ/ホイール', '自動車パーツ', '自動車アクセサリー', 'オートバイパーツ', 'オートバイアクセサリー']
car_bike_grandchildren_array = [
  ['国内自動車本体', '外国自動車本体'],
  ['タイヤ/ホイールセット', 'タイヤ', 'ホイール', 'その他'],
  ['サスペンション', 'ブレーキ', '外装/エアロパーツ', 'ライト', '内装品/シート', 'ステアリング', 'マフラー/排気系', 'エンジン/ミッション/駆動系', '電装品', '補強パーツ', '汎用パーツ', '外国自動車用パーツ', 'その他'],
  ...
  ['ヘルメット/シールド', 'バイクウェア/装備', 'アクセサリー', 'メンテナンス', 'カタログ/マニュアル', 'その他']
]

car_bike_children_array.each_with_index do |children, index|
  children = car_bike.children.create(name: children)
  car_bike_grandchildren_array[index].each do |grandchildren|
    children.children.create(name: grandchildren)
  end
end

  # その他
other_children_array = ['ペット用品', '食品', '飲料/酒', '日用品/生活雑貨/旅行', 'アンティーク/コレクション', '文房具/事務用品', '事務/店舗用品']
other_grandchildren_array = [
  ['ペットフード', '犬用品', '猫用品', '魚用品/水草', '小動物用品', '爬虫類/両生類用品', 'かご/おり', '鳥用品', '虫類用品', 'その他'],
  ['菓子', '米', '野菜', '果物', '調味料', '魚介類(加工食品)', '肉類(加工食品)', 'その他加工食品', 'その他'],
  ['コーヒー', 'ソフトドリンク', 'ミネラルウォーター', '茶', 'ウィスキー', 'ワイン', 'ブランデー', '焼酎', '日本酒', 'ビール/発泡酒', 'その他'],
